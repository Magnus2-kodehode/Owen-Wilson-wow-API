  ...
  const fetchOrderedWows = async () => {
    ...
    try {
      const response = await fetch(
        `https://owen-wilson-wow-api.onrender.com/wows/ordered/${range.start}-${range.end}`
      )
  ...
  return (
    <div className='main'>
      <div className='container-main'>
        <p className='title'>Browse all wows</p>
        ...
        <div className='wow-nav'>
          <button onClick={prevPage} disabled={range.start === 0} className='wow-nav-button'>
            Prev page
          </button>
          <button onClick={nextPage} className='wow-nav-button'>
            Next page
          </button>
        </div>
      </div>
